...
template<class T> void NesPpu<T>::UpdateStatusFlag()
{
	...
	if(_nmiSuppressRaceEnabled && _scanline == _nmiScanline && _cycle == 0) {
		//"Reading one PPU clock before reads it as clear and never sets the flag or generates NMI for that frame."
		//A 2C02 race; some famiclone PPUs (e.g. the BBK's) clear the vblank flag on reads
		//but do NOT have this exact suppression - software that polls $2002 in a tight loop
		//would otherwise lose an occasional NMI (a one-frame glitch at screen transitions).
...
template<class T> void NesPpu<T>::LoadTileInfo()
{
	switch(_cycle & 0x07) {
		...
		case 1: {
			...
			if(_splitBgFetchMode) {
				_mapper->ApplySplitBgFetch(tileIndex, _videoRamAddr, (uint16_t)_cycle, _tile.TileAddr);
			}
		...
		case 7:
			//The YuXing 2-screen split is 1bpp - the chip feeds the same byte to both
			//planes, so every set bit comes out as color 3. The 4-band split is ordinary.
			_tile.HighByte = ReadVram(_tile.TileAddr + (_splitBgFetchMode == 1 ? 0 : 8));
			break;
	}
}
